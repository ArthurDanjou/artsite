...
ArtLab is a multi-host homelab orchestrated by **Dockhand**, a Docker management UI running on the central server. Three remote hosts are attached to it through **Hawser** agents in Edge mode, using outbound-only WebSocket connections, with zero inbound ports wherever the host lives.
...
**AdGuard Home** is the DNS for the whole tailnet (global nameserver) and for the LAN (through the router's DHCP). On the public side, everything goes through **Traefik v3**: TLS is terminated at the proxy with Let's Encrypt certificates issued via the Cloudflare DNS challenge, so no ports to open for ACME.
...
This homelab is a sandbox for DevOps experimentation, infrastructure reliability, and keeping my digital life private.
