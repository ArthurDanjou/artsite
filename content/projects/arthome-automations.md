...
ArtHome runs as a Home Assistant OS instance on the Beelink EQR6, exposed through a **Cloudflare Tunnel** for secure remote access. All local traffic stays inside a **Tailscale mesh VPN**, with no open ports and no exposed attack surface.
...
| Add-on | Role |
| :--- | :--- |
| **Zigbee2MQTT** | Universal Zigbee bridge that controls all Hue, IKEA, and Aqara devices locally |
...
If nobody has been home for **24 hours** and guest mode is off, **vacation mode** activates automatically and notifies both residents. It disables on first return.
...
The system tracks daily hours at Polytechnique: after **8 hours**, Arthur receives a "time to go home" notification (workdays only, once per day; the flag resets at midnight). When Arthur leaves campus between 15:00 and 22:00, Nerea is notified automatically.
